        ...
        <v-layout row wrap>
            <v-flex md8>
             ...
             <v-flex md4 >
                    ...
                    <!-- <h4>concepts</h4> -->
                    <div :key="index" v-for="(todo, index) in fabian">
                           <!-- {{todo.type}} -->
                       ...
                       <v-card v-model="rabbi" class="headline"  :key="index" v-for="(element, index) in todo">
...
methods: {
  ...
  .then(res => this.todos = res.data.named_entitites)
  .catch(err => alert(err));
     ...
     setCurrent: _.debounce(function() {
         ...
         this.items = ""
       axios.get("http://158.39.77.36:5000/story/refresh1?storyid=1234abcd0987fedc&userid=abcd1234fedc0987")
  .then(res => this.fabian = res.data.concepts)
  .catch(err => alert(err));
    this.newscodes= this.dam
     }, 2000), 
